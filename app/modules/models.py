        ...
        if cursor.fetchone()[0] == 0:
            ...
            cursor.execute("""
                CREATE TABLE "settings" (
                    ...
                    "default_preset"	INTEGER DEFAULT 1,
                    ...
                    "horizontal_scroll_enabled"	TEXT DEFAULT 'TRUE',
                    "horizontal_scroll_speed"	INTEGER DEFAULT 1,
                    "horizontal_scroll_delay"	INTEGER DEFAULT 60000,
                    "vertical_scroll_enabled"	TEXT DEFAULT 'TRUE',
...
def migrate_db(db_path):
    ...
    if current_version < 3:
        # players.hidden (player show/hide) only ever landed on fresh installs -
        # init_db's CREATE TABLE has always included it, but no migration ever added
        # it to an existing table. Every query in players.py reads/writes it
        # unconditionally, so a database missing this column throws on ordinary
        # scoreboard page loads, not just an edge case.
        existing_columns = {row[1] for row in cursor.execute("PRAGMA table_info(players)")}
        if "hidden" not in existing_columns:
            cursor.execute("ALTER TABLE players ADD COLUMN hidden TEXT NOT NULL DEFAULT 'FALSE'")

        # The 3 non-Default presets similarly only land when the presets table is
        # empty at first boot (init_db). INSERT OR IGNORE on the unique name column
        # means this is a no-op for anyone who already has all 4, and never touches
        # a user's own preset if they happened to reuse one of these names.
        cursor.executemany("""
            INSERT OR IGNORE INTO presets (
                name, css_body, css_card, css_score_cards, css_initials, css_scores, css_box, css_title
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?);
        """, [
            (
                "Neon Glow",
